...
const MemoryGame = () => {
	const [player, setPlayer] = useState(JSON.parse(localStorage.getItem("player")) !== null ? JSON.parse(localStorage.getItem("player")) : []);
	...
	const realPlayers = player.filter((p) => {return p.isPlayer})
	...
	useEffect(() => {
		if (matchingCards.length === 4) {
			setTimeout(() => {
				const shuffledImg4x4 = FYShuffle([...cardImg4x4, ...cardImg4x4]).map(value => ({...value, id: Math.random() * 100}));
		...
		if (matchingCards.length === 20) {
			setTimeout(() => {
				const shuffledImg6x6 = FYShuffle([...cardImg6x6, ...cardImg6x6]).map((value) => ({ ...value, id: Math.random() * 100 }));
		...
		if (matchingCards.length === 56) {
			setTimeout(() => {
				const shuffledImg8x8 = FYShuffle([...cardImg8x8, ...cardImg8x8]).map((value) => ({ ...value, id: Math.random() * 100 }));
	...
	const shuffleCards = () => {
		const shuffledImg = [...cardImg2x2, ...cardImg2x2]
	...
	const playAgain = () => {
		...
		const shuffledImg2x2 = [...cardImg2x2, ...cardImg2x2]
			.sort(() => Math.random() - 0.5)
	...
	return (
		<div className="memory_game_container">
			...
			<div className="game_board">
				...
				{showModal && moves <= realPlayers[realPlayers.length-1].numberOfTurns && (
					<Modal
						moves={moves}
